...
> To not **shard** the validator set among different blockchain, but **share** the validator set among different blockchains, while sharding the execution of blocks as much as possible. 
...
> 
> How Polkadot and ELVES achieve this as by deploying a mix of sequential blockchain and a [[Scaling Out - Pure Multi-chain]] system: At first, in each shard is managed by a smaller group of validators (currently 5), much like a [[Scaling Out - Pure Multi-chain]] system. Then, the system progressively moves towards a sequential model, where a randomized group of validators from the broad validator set re-check this work. In the absence of any detection of fraud, this process terminates with as little of 30 other validators re-checking the work of the first 5. In the presence of fraud, the system quickly escalates into a fully sequential model. 
> 
> With the current implementation, Polkadot is estimated to be able to suppose up to 100 shards with 1000 validators, and in [[JAM]] this number is estimated to be 341.
...
## [[The Layers Terminology|Layer-2]]-2 Blockchains
...
A good mental model for this is the consider the above diagram, but with one major difference: Each of those shards being a different blockchain. These blockchains are then called Layer-2s/L2s in the Ethereum terminology, or Parachain in Polkadot's. 

![[Scaling Out - Shared Economic Security 2025-12-22-17.54.37.excalidraw]]

Notice have we have clearly identified that the L1 is its own blockchain, having its own sequence of blocks. Consequently, each L2 also has its own sequence of blocks, and one way or another, these blocks are being linked back to L1 blocks.
...
- First, recall our the base-blockchain and its secure validator set is called the L1. 
- We acknowledge that we cannot have the classical [[Introduction - Why Scaling Matters#Sequential Blockchains|sequential model]] model where a single validator in L1 produces a block with all of the work done in it, and all other validators re-execute it.
- Instead, we rely on a new model where some work (e.g. producing a new L2 [[Block]]) is happening by an entity outside of the validators set of L1.
...
Crucially, for the L2 to be able to achieve the last point above, it must:
1. The L1 validators need be able to re-construct the data to inspect the work done by L2 for some period of time, and potentially re-execute it or settle any issues in it. This is solved by a system called the [[Data Availability]] layer that all L1 blockchains have to provide.
