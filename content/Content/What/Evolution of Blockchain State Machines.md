...
In Ethereum, the **Ethereum Virtual Machine** (**EVM**) byte-code is used. The default language that could be compiled to EVM was **Solidity**, yet today more languages can nowadays be compiled to EVM byte-code.

> [!note]- First Mover Advantage Of Solidity
> By and large, the majority of the blockchain ecosystem is focused on using Solidity as the main programming language of extending blockchains[^9]. Yet, a lot of interesting effort has also been put into allowing other programming languages to be used to write smart-contracts. In some sense, Solidity is the Javascript of Web3; it is not perfect, but being first made it almost impossible to fully replace. 
...
This allowed Polkadot to not only host smart-contracts (with relatively limited programmability), but also host an entire set blockchains within itself, giving birth to the idea of a interconnected multi-chain blockchain networks. This ideology was later reinforced by Ethereum and its [rollup-centric-roadmap](https://ethereum-magicians.org/t/a-rollup-centric-ethereum-roadmap/4698) and exists to this day the most accepted way to **scale** blockchains. A [[Rollup]] is a secondary blockchain that runs on top of Ethereum, helps it scale, and derives it [[Trustless]]ness in part from Ethereum. [[Polkadot]] and Ethereum do this in different ways, with different consequences about how secure the [[Rollup]] is, which we will discuss in the [[Introduction - Why Scaling Matters|scaling]] chapters.

> In this context, the *hosting blockchain* is often called the Layer-1 or L1 blockchains, and the blockchains being *hosted* (the [[Rollup]]s) are called Layer-2 or L2 blockchains. See [[The Layers Terminology]].
#### Multi-chain Blockchain Ecosystems
In some sense, multi-chain designs are a middle-ground between [[#Smart Contracts]], and [[#Custom Blockchains]]. A multi-chain ecosystem (such as Polkadot or Ethereum) offer solutions to the problems named above about [[#Custom Blockchains]], and in return offer greater flexibility and scalability to the entire ecosystem. The solutions being:
- SDKs and standards to make building L2s easier (such as the OP-Stack in and `polkadot-sdk` for Ethereum and Polkadot respectively)
...
Innovation in [[#Hosting Any Application (With Any Runtime Duration)]] has sparked the **[[Web3]] Could** narrative. If blockchains manage to make their offerings, such as computation and storage, more and more flexible, it will start to look quite similar to how the Cloud industry works today, except the primitives offered in the Web3 cloud would likely have differing properties.
...
### Metering and Gas
As mentioned in the [[#Web3 Cloud Narrative]], an interesting side-effect of allowing smart contracts to run on blockchains is that the system starts to resemble the internet cloud companies that we know today; there is a **infrastructure provider**, on top of which you purchase your own VM, or deploy a server-less code.
...
What if your server-less code starts maxing out the CPU/GPU to mine cryptocurrencies? how would the cloud provider do to protect themselves? they charge you more money. 

A smart-contract blockchain must have the same measure in place to survive abuse. Moreover, a smart-contract blockchain's measure to protect itself should arguably be a bit more draconian, because it intends to be a fully permissionless (one of the pillars of being [[Trustless]]) system to which anyone can upload any code. This is unlike a traditional cloud provider that verifiers its users via email, collects their banking information and so on.
...
If you ever interact with an Ethereum wallet, you might see signs of "gas" when you submit transactions. This is the upper-bound on the gas amount that the user "promises" its execution of a contract will take and is willing to pay for it. The gas amount is then converted to an ETH equivalent, and is charged from the user in return for the execution.

> [!note]- Sponsored Transactions
> The gas fees have been a major hurdle for [[Web3]] adoption so far. Imagine you would have to pay 10 cents every time you would send a message to your friends; no one would use such a messenger. There are two paths forward for this, both being explored by various blockchains: 
> 
...
## Summary
This is likely the longest chapter of this book, and explored what the different types of blockchain [[STF]]s that we have so far seen. Recall that the [[STF]] of a blockchain is the code that defines how the blockchain should execute a new incoming [[Block]], and potentially update its [[State]] based on it.
...
[^9]: [[Blocks, Transactions, And Blockchain Systems#Node Software / Client|Blockchain node software]] are often written in high performant, low-level languages such as Rust and C++.
