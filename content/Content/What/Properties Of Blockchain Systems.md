All of the chapters so far has given us a solid foundation to understand *what* blockchain systems can do, and to a limited extend, *how* they do it. In the remaining closing chapters of part 1, we will zoom back and evaluate the bigger picture. 
- First, in this chapter, we will (re-)iterate what properties the system that we have so far described have. 
- In the next chapter, [[Content/What/The Bigger Picture]], we will discuss what other technologies are necessary to be exist alongside a blockchain to deliver a useful system to the people of the world that actually helps them live a life with less [[Trust#Human-based Trust|Human-based Trust]] and more [[Trust#Science-based Trust|Science-based Trust]]. 
...
- Auditable History: The entire history of all computations that has happened can be audited, and the intermediate states after each step can be reconstructed.
...
- **Sensitive** applications where the accessibility of a [[Trustless]] system is desired, such as whistleblowing. Imagine use-cases where it is desirable to know that once a data or [[Smart Contract]] is published, it cannot be taken down by any single individual or authority.
## Public 
...
We emphasize "by default" because there are techniques to make this be partially or fully private, but this involves cryptography primitives (see [[Moon Math - ZK, FHE and MPC]]) and it is not the norm. As of today, almost all major blockchains operate fully in public, where every transaction and its details can be seen by anyone. Specific smart contracts attempt to provide privacy to users[^3], and a handful of blockchains are designed from the ground up to be private[^4]. 

> [!warn]- Block Explorers 
> This is why every blockchain has a notion of _Block Explorer_, a public dashboard where every block and every transaction and every account's activity is indexed. None of your banks have a public explorer where you can see the money that your neighbor spend.

The main remedy to this in public blockchains, so far, has been using pseudonym accounts. In that a user's identity is not mandatory linked to an account-id that the blockchain recognizes[^2]. If care is taken, it is possible to keep an account-id anonymous. So, even though anyone can see *what* an account does, they cannot easily know *who* it is. Yet, with limited [[On and Off Ramp]] options, it is almost always the case that the centralized exchanges can always link an identity to an account-id.

> It is often said that having no privacy is the "original sin" of Bitcoin.
## Digital 
This point has been well emphasized as early [[Blockchain-based Authorities#Oracle Problem|the second chapter]], but perhaps it is easier to re-iterate it here: The full [[Trustless]] spectrum of blockchains can be best materialized if the [[State]] on top of which the blockchain is coming to consensus is fully digital, and has no dependency on an [[Oracle Problem|oracle]] to bridge real world information to the blockchain. This is not to say that no such endeavor should be attempted, but it is something important to be aware of. 
## Ownerless
one of the properties of [[Trust#Science-based Trust|Science-based Trust]] is that they require much less support to operate. When you forget your Gmail password, you can contact google and ask them to recover it for you, because end of the day they are the gatekeepers. If you looks your digital private key used in Ethereum, unfortunately there is no one to contact. 

This is a double-edge sword; [[Trustless]] systems have fewer (if any) humans to control them. It makes them more resilient to corruption, but it also gives them this sense of being "Ownerless".
...
This is somewhat of a pain point for the adoption of [[Web3]] technologies. Unsurprisingly no one feels comfortable to do business knowing there is no support hotline to fix issues if something goes wrong. Needless to say, many blockchain protocols have some form of support baked into them, either as a supporting company or foundation, or the [[Governance]] system having promises. 

But, end of the day, it is a new paradigm, and have to change our perspectives to trust the system rather than the supporting humans behind it. 
## Summary
Blockchains provide [[Trustless]] computation and storage primitives to developers to build applications that inherit those Trustless properties. 

As of now, blockchains do this in public, but this is not a fundamental limitation and with further performance improvements in the [[Moon Math - ZK, FHE and MPC]] group of technologies, they could operate with much more privacy. 

They suffer from a high cost and slow speed to achieve the Trustless properties. At first sight, it is clear that use-cases that are social, contentious and digital are best fitted to be implemented by blockchains. 
